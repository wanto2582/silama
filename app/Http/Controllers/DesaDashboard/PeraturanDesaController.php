...
class PeraturanDesaController extends Controller
{
    ...
    {
        $peraturanDesa = PeraturanDesa::latest()->get();

        return view('desa.peraturan_desa.index', compact('peraturanDesa'));
    }
    ...
    {
        return view('desa.peraturan_desa.form', [
            'peraturanDesa' => new PeraturanDesa(),
            'metapage' => [
                'title' => 'Tambah File',
                'url' => route('desa.peraturan-desa.store'),
                'method' => 'POST',
                'button' => 'Add'
            ]
        ]);
    ...
    public function store(Request $request)
    {
        ...
        $data = [
            ...
            // Simpan file ke disk 'public' dalam folder 'assets/peraturan_desa'
            'file' => $request->file('file')->store('assets/peraturan_desa', 'public'),
        ];

        PeraturanDesa::create($data);

        Alert::success('Sukses!', 'Data Berhasil Dibuat');
        return redirect()->route('desa.peraturan-desa.index');
    }

    
        public function show(string $id)
            {
        $peraturanDesa = PeraturanDesa::findOrFail($id);

         // Ini akan langsung menampilkan PDF di browser
        if ($peraturanDesa->file && Storage::disk('public')->exists($peraturanDesa->file)) {
        $filePath = Storage::disk('public')->path($peraturanDesa->file);
    ...
    {
        $detailSurat = PeraturanDesa::where('id', $id)->first();
        return view('desa.peraturan_desa.edit', compact('detailSurat'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            // Simpan file baru
            $filePath = $request->file('file')->store('assets/peraturan_desa', 'public');
        }
    ...
    {
        try {
            // Find dokumen Peraturan Desa by ID
            $peraturanDesa = PeraturanDesa::findOrFail($id);

            // Delete file if exists
            if ($peraturanDesa->file) {
                // Gunakan Storage::disk('public')->delete() untuk menghapus file
                if (Storage::disk('public')->exists($peraturanDesa->file)) {
                    Storage::disk('public')->delete($peraturanDesa->file);
                }
            }
            ...
            if ($request->ajax()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Gagal menghapus dokumen Peraturan Desa: ' . $e->getMessage()
                ], 500);
            }

            Alert::error('Error!', 'Gagal menghapus dokumen Peraturan Desa: ' . $e->getMessage());
            return redirect()->route('desa.peraturan-desa.index');
        }
    }

    public function __datatable_peraturan_desa(Request $request)
    {
        $query = PeraturanDesa::latest();
        ...
        return DataTables::of($query)->addIndexColumn()
            ->addColumn('action', function ($model) {
                ...
                if ($model->file) {
                    $viewURL = route('desa.peraturan-desa.show', $model->id);
                    $view_btn = "
                ...
                // Edit button
                $editURL = route('desa.peraturan-desa.edit', $model->id);
                $edit_btn = "
