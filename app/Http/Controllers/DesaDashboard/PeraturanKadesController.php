...
class PeraturanKadesController extends Controller
{
    /**
    ...
    {
        $PeraturanKades = PeraturanKades::latest()->get();

        return view('desa.peraturan_kades.index', compact('PeraturanKades'));
    }
    ...
    {
        return view('desa.peraturan_kades.form', [
            'PeraturanKades' => new PeraturanKades(),
            'metapage' => [
                'title' => 'Tambah File',
                'url' => route('desa.peraturan-kades.store'),
                'method' => 'POST',
                'button' => 'Add'
            ]
        ]);
    ...
    public function store(Request $request)
    {
        ...
        $data = [
            ...
            // Simpan file ke disk 'public' dalam folder 'assets/peraturan_kades'
            'file' => $request->file('file')->store('assets/peraturan_kades', 'public'),
        ];

        PeraturanKades::create($data);

        Alert::success('Sukses!', 'Data Berhasil Dibuat');
        return redirect()->route('desa.peraturan-kades.index');
    }

    
        public function show(string $id)
            {
        $PeraturanKades = PeraturanKades::findOrFail($id);

         // Ini akan langsung menampilkan PDF di browser
        if ($PeraturanKades->file && Storage::disk('public')->exists($PeraturanKades->file)) {
        $filePath = Storage::disk('public')->path($PeraturanKades->file);
    ...
    {
        $detailSurat = PeraturanKades::where('id', $id)->first();
        return view('desa.peraturan_kades.edit', compact('detailSurat'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            // Simpan file baru
            $filePath = $request->file('file')->store('assets/peraturan_kades', 'public');
        }
    ...
    public function destroy(Request $request, string $id)
    {
        try {
            ...
            if ($PeraturanKades->file) {
                // Gunakan Storage::disk('public')->delete() untuk menghapus file
                if (Storage::disk('public')->exists($PeraturanKades->file)) {
                    Storage::disk('public')->delete($PeraturanKades->file);
                }
            }

            // Delete record from database
            $PeraturanKades->delete();
    ...
    public function __datatable_peraturan_kades(Request $request)
    {
        $query = PeraturanKades::latest();
        ...
        return DataTables::of($query)->addIndexColumn()
            ->addColumn('action', function ($model) {
                ...
                if ($model->file) {
                    $viewURL = route('desa.peraturan-kades.show', $model->id);
                    $view_btn = "
                ...
                // Edit button
                $editURL = route('desa.peraturan-kades.edit', $model->id);
                $edit_btn = "
